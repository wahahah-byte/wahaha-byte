...
import { canCheckInNow, dateKey, getNextOccurrenceLabel, getUnlockInfo, parseLocalDate, isOverdue, todayLocalKey, isCycleClosed } from "@/lib/dateUtils";
...
function TaskRowImpl({
  ...
  filingIds, errorIds, selectedIds, submittedTaskIds,
  ...
  isOpenInDetail, useCheckinCheckbox,
}: TaskRowProps) {
            ...
            const label = wasCheckedInToday
              ...
              : filledThisCycle
                ...
                : isActionable
                  ? "Check in"
                  : overdueRecurring
                    ? "Overdue — reschedule from the detail panel"
                    : "Not yet available";
            const onBoxClick = (e: React.MouseEvent) => {
              e.stopPropagation();
              if (wasCheckedInToday && undoableCycle) {
                onUndoCheckIn?.(task, undoableCycle.cycleId);
                return;
              }
