...
export default function LoginPage() {
  ...
  const [tab, setTab] = useState<"custom" | "clerk">("custom");
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#313338]">
      <div className="w-full max-w-md bg-[#2B2D31] rounded-lg shadow-2xl overflow-hidden">
        ...
        {/* Tab bar */}
        <div className="flex bg-[#232428]">
          {(["custom", "clerk"] as const).map((t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
            ...
            >
              {t === "custom" ? "Email & Password" : "Sign in with Clerk"}
            </button>
          ))}
        </div>
              ...
              <SignIn
                appearance={{
                  ...
                  elements: {
                    ...
                    headerSubtitle: "text-[#B5BAC1]",
                    formButtonPrimary:
                      "bg-[#5865F2] hover:bg-[#4752C4] text-white",
                    formFieldInput:
                      "bg-[#1E1F22] border-transparent focus:border-[#5865F2] text-[#DBdEE1]",
